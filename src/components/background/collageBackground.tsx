...
export default function CollageBackground({ children }: BackgroundProps) {
    console.log("fundo->", styleTypes[`${process.env.STYLE}`].bg);
    const bgList = styleTypes[`${process.env.STYLE}`].bg;

    return (
        <main
            className={`w-screen h-screen overflow-y-auto p-0 relative bg-[#ebd4ba]`}
        ...
        >
            <div
                className="fixed w-screen h-screen bg-center mix-blend-overlay"
                style={{
                    backgroundImage: `url(${bgList[1]})`,
                }}
            />
            <div
                className="fixed w-screen h-screen bg-center bg-contain mix-blend-screen"
                style={{
